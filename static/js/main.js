...
async function fetchBestFilm(link, film_number){
	let lien =(url+link);
	const response = await fetch(lien);
  	const myJson = await response.json();
	myJson.results.forEach(element =>{
		affichage_meuilleur_film.src = (myJson.results[film_number].image_url);
	})
...
// Création de la premiére section, faire le fetch des images dans l'API et les inserer à la carrousel
async function filmsMieuxNote(link,page_number,_link_comp){
	num = 0
...
async function filmsAnimation2020(link,page_number,_link_comp){
    for (let i = 1; i < page_number; i ++){
		let lien = (url+link+i)
			const response = await fetch(lien);
  			const myJson = await response.json();
...
async function filmsAction(link,page_number,_link_comp){
    for (let i = 1; i < page_number; i ++){
		let lien = (url+link+i)
			const response = await fetch(lien);
  			const myJson = await response.json();
...
async function filmsBiography(link,page_number,_link_comp){
    for (let i = 1; i < page_number; i ++){
		let lien = (url+link+i)		
			const response = await fetch(lien);
  			const myJson = await response.json();
...
// Appel des fonctions
fetchBestFilm("/titles/?imdb_score_min=9.6&page=1",0);
filmsMieuxNote("/titles/?imdb_score_min=9&page=",4);
filmsAnimation2020("/titles/?genre=Animation&imdb_score=&imdb_score_max=&imdb_score_min=8.5&page=",5);
filmsAction("/titles/?genre=Action&imdb_score=&imdb_score_max=&imdb_score_min=8.8&page=",4)
filmsBiography("/titles/?genre=Biography&imdb_score_min=8.5&page=",5)